...
def draw_time(canvas, x, y):
    """
    Creates an enhanced time and date display on the canvas
    ...
    # Create glossy panel effect
    panel_width = 210
    panel_height = 80
    
    # Draw panel with gradient
    for i in range(panel_height):
        alpha = i / panel_height  # Gradient factor
        color = (int(20 + 5 * alpha), int(20 + 5 * alpha), int(30 + 10 * alpha))
        cv2.line(canvas, (x-10, y-30+i), (x+panel_width-10, y-30+i), color, 1)
    
    # Panel border with gloss effect
    cv2.rectangle(canvas, (x-10, y-30), (x+panel_width-10, y-30+panel_height), (100, 100, 100), 1)
    # Top gloss
    cv2.line(canvas, (x-9, y-29), (x+panel_width-11, y-29), (150, 150, 150), 1)
    
    # Digital clock style text
    # Time with shadow
    cv2.putText(canvas, "TIME:", (x, y), 
               cv2.FONT_HERSHEY_SIMPLEX, 0.6, (100, 100, 100), 1)
    cv2.putText(canvas, current_time, (x+70, y), 
               cv2.FONT_HERSHEY_SIMPLEX, 0.8, (100, 100, 240), 1)
    cv2.putText(canvas, current_time, (x+70, y), 
               cv2.FONT_HERSHEY_SIMPLEX, 0.8, (150, 150, 255), 2)
    
    # Date with shadow
    cv2.putText(canvas, "DATE:", (x, y+40), 
               cv2.FONT_HERSHEY_SIMPLEX, 0.6, (100, 100, 100), 1)
    cv2.putText(canvas, current_date, (x+70, y+40), 
...
def create_enhanced_ui(video_frame, parking_map, WINDOW_WIDTH=1200, WINDOW_HEIGHT=700, 
    ...
    # Add time display
    draw_time(ui_canvas, WINDOW_WIDTH - 250, 100)
